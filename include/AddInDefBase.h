    ...
    public IInterface
{
    ...
    virtual bool ADDIN_API QueryBox(const WCHAR_T* queryText, 
        Button* buttons, const unsigned int sizeButtons, tVariant* retVal,
        const unsigned long timeout = 0, 
        const unsigned int defButton = 0, 
        const WCHAR_T* caption = 0, 
        const unsigned int timeoutButton = 0) = 0;

    virtual bool ADDIN_API MessageBox(const WCHAR_T* text, tVariant* retVal,
        const unsigned long timeout = 0,
        const WCHAR_T* caption = 0) = 0;
};
